...
function NutritionistCreateDietPlanDay({
    ...
    const deleteMeal = (recipe: Recipe, recipeIndex: number) => {
        const newMeals = meals.filter((m: any, index: number) => index !== recipeIndex);
        ...
        if (!queuedMeal) {
            if (recipe.attributes.servings > 1 && recipe.attributes.meal_type.includes('Prato Principal')) {
                queueMeal({
                    meal: recipe,
                    unusedServings: 1,
        ...
        if (queuedMeal && queuedMeal.unusedServings + 1 === queuedMeal.meal.attributes.servings) {
            queueMeal(undefined);
            if (recipe.attributes.meal_type.includes('Prato Principal')) changeMainCourses(-1);
        } else if (queuedMeal) {
            queueMeal({
                ...queuedMeal,
                ...
                meals
                    .map((m: Recipe) => {
                        if (m?.attributes?.nutritional_values?.['carbohydrate']?.portion)
                            return parseInt(m?.attributes.nutritional_values?.['carbohydrate']?.portion?.split(' ')[0]);
                        return null;
                    })
                    .reduce((a: number, b: number) => a + b),
            );
    ...
    const MealContainer = ({ recipe, recipeIndex }: any) => {
        ...
        return (
            <Box sx={ableToDelete() ? styles.fadedMealContainer : styles.mealContainer}>
                ...
                <Box>
                    ...
                    {meals.map((r: Recipe, index: number) => (
                        <Box sx={styles.mealsWrapper} key={'mealsWrapper' + weekDay + index}>
                            <MealContainer key={index + weekDay + r.id} recipe={r} recipeIndex={index} />
