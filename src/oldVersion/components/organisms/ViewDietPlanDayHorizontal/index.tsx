...
import { DietPlanAttributes, getWeekDay, Plan, planKeys } from '../../../util/dietPlans';
...
function ViewDietPlanDayHorizontal({ dietPlan, weekDay }: ViewDietPlanDayHorizontalProps) {
    ...
    useEffect(() => {
        if ((dietPlan?.plan as Plan)[weekDay as keyof planKeys]) {
            fetchMeals();
        }
    ...
    const MealContainer = ({ recipe }: MealContainerProps) => {
        return (
            <Box sx={styles.mealContainer}>
                <Typography variant="body1" sx={{ mb: 1, ml: 1, fontWeight: 'bold', fontSize: '16px' }}>
        ...
        <Box>
            <Box sx={styles.wrapper}>
                <Box sx={styles.header}>
                    ...
                    <Box sx={{ display: 'flex' }}>
                        <Typography id="modal-modal-title" variant="h6" component="h2" sx={{ mr: 3 }}>
                        ...
                        <Typography id="modal-modal-title" variant="h6" component="h2" sx={{ mr: 0 }}>
