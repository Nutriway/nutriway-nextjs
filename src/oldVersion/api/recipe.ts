...
export const getNutritionistRecipesByNutritionistId = async (
...
export const getApiRecipes = async (jwt: string) => {
  try {
    const { data } = await api(jwt).get(
      `api-recipes?populate=*&pagination[pageSize]=10000`
    );
    return data;
  } catch (error) {
...
export const getApiRecipeByRecipeId = async (
  id: string,
  jwt: string
) => {
  try {
...
export const getNutritionistRecipeByRecipeId = async (
  id: string,
  jwt: string
) => {
  try {
    const { data } = await api(jwt).get(
      `nutritionist-recipes/${id}`
    );
    return data;
  } catch (error) {
    toast.error("Erro ao procurar a receita");
...
export const getNutritionistRecipesByRecipesIds = async (
