...
export default function NutritionistCalendar({ appointments }: NutritionistCalendarProps) {
    ...
    const selectedDayMeetings = appointments.filter((consultation) =>
        isSameDay(parseISO(consultation.attributes.date), selectedDay),
    );
                    ...
                    <section className="mt-12 w-80 md:pl-14 md:mt-0">
                        <h2 className="font-semibold text-gray-900">
                            ...
                            <time dateTime={format(selectedDay, 'yyyy-MM-dd', { locale: pt })}>
                                {format(selectedDay, 'MMMM dd, yyy', { locale: pt })}
                            </time>
                        </h2>
    ...
    return (
        <li className="flex items-center py-2 px-4 space-x-4 rounded-xl focus-within:bg-gray-100 hover:bg-gray-100 group">
            ...
            <div className="flex-auto">
                ...
                <p className="mt-0.5">
                    <time dateTime={consultation.attributes.date}>
                        {format(startDateTime, 'HH:mm ', { locale: pt })}
                    </time>
                    ...
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-36 bg-white rounded-md ring-1 ring-black ring-opacity-5 shadow-lg origin-top-right focus:outline-none">
                        <div className="py-1">
                            <Menu.Item>
                                {({ active }) => (
                                    <Link
                                        href={`/appointment/${consultation.id}`}
                                        className={`${
